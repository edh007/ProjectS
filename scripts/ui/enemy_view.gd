...
var _combatant: EnemyCombatant = null


func bind(enemy: EnemyCombatant) -> void:
	_combatant = enemy
...
func _update_intent(move: EnemyMove) -> void:
	if move == null:
		_intent_label.text = "Intent: ?"
		return
	match move.intent_icon:
		"attack":
			var dmg: int = move.move_params.get("damage", 0)
			var hits: int = move.move_params.get("hits", 1)
			if hits > 1:
